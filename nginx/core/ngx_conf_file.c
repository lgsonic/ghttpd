...
ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    if (filename) {
        ...
        fd = ngx_open_file_map(filename->data, NGX_FILE_RDONLY, NGX_FILE_OPEN, 0);
...
ngx_conf_read_token(ngx_conf_t *cf)
{
    ...
    file_size = ngx_file_size_map(&cf->conf_file->file.info);
    ...
    for ( ;; ) {
        ...
        if (b->pos >= b->last) {
            ...
            n = ngx_read_file_map(&cf->conf_file->file, b->start + len, size,
