...
static ngx_uint_t  ngx_listen_port = 80;


static char **ngx_os_environ;

static u_char *ngx_conf_data = "worker_processes  1;\r\n\
events {\r\n\
...
main(int argc, char *const *argv)
{
	...
	ngx_file_t  conf_file;
	u_char conf_buf[4096];
        ...
        if (ngx_show_help) {
            ngx_log_stderr(0,
                ...
                "  -g directives : set global directives out of configuration "
                                   "file" CRLF
				"  -l listen_port: set listen_port (default: 80)" CRLF
                );
        }
	...
	// init conf file map
	conf_file.fd = ngx_create_file_map(init_cycle.conf_file.data);
	memset(conf_buf, 0, 4096);
	ngx_snprintf(conf_buf, 4096, ngx_conf_data, ngx_listen_port);
	ngx_write_file_map(&conf_file, conf_buf, 4096, 0);

    cycle = ngx_init_cycle(&init_cycle);

	// release conf file map
	ngx_release_file_map(init_cycle.conf_file.data);
...
*/
	...
	ngx_single_process_cycle(cycle, 1);
...
//#ifdef _USRDLL
int ghttpd_start(int nListenPort)
{
    ngx_int_t         i;
    ngx_log_t        *log;
    ngx_cycle_t      *cycle, init_cycle;
	...
	conf_file.fd = ngx_create_file_map(init_cycle.conf_file.data);
	memset(conf_buf, 0, 4096);
	ngx_snprintf(conf_buf, 4096, ngx_conf_data, nListenPort);
	ngx_write_file_map(&conf_file, conf_buf, 4096, 0);
...
*/
	...
	ngx_single_process_cycle(cycle, 0);
...
ngx_get_options(int argc, char *const *argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        while (*p) {
			...
			case 'l':
				if (*p) {
					ngx_listen_port = atoi(p);
					goto next;
				}

				if (argv[++i]) {
					ngx_listen_port = atoi((u_char *) argv[i]);
					goto next;
				}

				ngx_log_stderr(0, "option \"-l\" requires listen_port");
				return NGX_ERROR;
