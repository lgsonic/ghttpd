...
ngx_fd_t
ngx_create_file_map(u_char *name)
{
	ngx_fd_t    fd;

	fd = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, 4096, name);

	return fd;
}

void
ngx_release_file_map(u_char *name)
{
	ngx_fd_t    fd;
...
ngx_fd_t
ngx_open_file_map(u_char *name, u_long mode, u_long create, u_long access)
{
	ngx_fd_t    fd;
...
ssize_t
ngx_read_file_map(ngx_file_t *file, u_char *buf, size_t size, off_t offset)
{
	u_long      n;
	ngx_err_t   err;
	
	void *pMapView = MapViewOfFile(file->fd, FILE_MAP_READ, 0, 0, 4096);
	if (pMapView == NULL)
	{
		err = errno;
		ngx_log_error(NGX_LOG_ERR, file->log, err,
			"MapViewOfFile() \"%s\" failed", file->name.data);
		return NGX_ERROR;
	}

	n = size;
	memcpy(buf, (char*)pMapView+offset, size);
...
ssize_t
ngx_write_file_map(ngx_file_t *file, u_char *buf, size_t size, off_t offset)
{
	u_long      n;
	ngx_err_t   err;

	void *pMapView = MapViewOfFile(file->fd, FILE_MAP_WRITE, 0, 0, 4096);
	if (pMapView == NULL)
	{
		err = errno;
		ngx_log_error(NGX_LOG_ERR, file->log, err,
	...
	memset(pMapView, 0, 4096);
	memcpy((char*)pMapView+offset, buf, size);
